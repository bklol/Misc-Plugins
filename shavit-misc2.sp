...
bool IsEditClient[MAXPLAYERS+1];
int EditClient[MAXPLAYERS+1];
...
char RankTag[MAXPLAYERS + 1][64];
char cctag[MAXPLAYERS + 1][64];
...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_tag", Command_Clantag, "choose a tag");
	gH_TagCookie = RegClientCookie("shavit_clantag", "clantag setting", CookieAccess_Protected);
...
public void OnClientCookiesCached(int client)
{
	...
	GetClientCookie(client, gH_TagCookie, sSetting, 8);
	...
	{
		Taguse[client] = StringToInt(sSetting);
	}
...
public void OnClientPostAdminCheck(int client)
{
	...
	IsEditClient[client]= false;
	SQL_FetchPlayer(client);
}

public void SQL_FetchPlayer_CB(Database db, DBResultSet results, const char[] error, any data)
{
	...
	if (results.FetchRow())
	{
		results.FetchString(1,cctag[iClient], sizeof(cctag));
	}
}

void SQL_FetchPlayer(int client)
{
	char szQuery[512];
	FormatEx(szQuery, sizeof(szQuery), "SELECT * FROM `testplayer` WHERE `auth` = '%s'",g_szAuth[client]);
	g_dDatabase.Query(SQL_FetchPlayer_CB, szQuery, GetClientSerial(client));
}
...
void UpdateRankedPlayers()
{
	...
	g_dDatabase.Query(SQL_UpdateRankedPlayers_Callback, sQuery, 0, DBPrio_High);
}
...
{
	Menus_ShowTag(client);
	return Plugin_Handled;
}

void Menus_ShowTag(int client)
{
	char Buffer[128];
	Menu menu = new Menu(Handler_mianMenu);
	menu.SetTitle("Shavit头衔面板");
	
	Format(Buffer, sizeof(Buffer), "排名头衔:\n%s",RankTag[client]);
	menu.AddItem("ranktag", Buffer);
	
	if(!StrEqual(cctag[client],""))
	{
		Format(Buffer, sizeof(Buffer), "特殊头衔:\n%s",cctag[client]);
		menu.AddItem("cctag",Buffer);
	}
	else
		menu.AddItem("cctag","自定义头衔:\n无",ITEMDRAW_DISABLED);
		
	if(CheckCommandAccess(client, "", ADMFLAG_KICK))
		menu.AddItem("admin","管理头衔:\n[Admin]");

	if(CheckCommandAccess(client, "", ADMFLAG_ROOT))
		menu.AddItem("admin","添加特殊头衔");
...
public int Handler_mianMenu(Menu menu, MenuAction action, int client,int itemNum)
{
	...
	if (action == MenuAction_Select)
	{
		...
		Menus_ShowTag(client);
		IntToString(itemNum, strID, sizeof(strID));
		SetClientCookie(client, gH_TagCookie, strID);
...
void EditMenu2(int client)
{

	Menu menu = new Menu(MenuHandler_Playeredit);
	SetMenuTitle(menu, "选择一个玩家");
	for(int i = 1; i <= MaxClients; i++)
	{
...
public int MenuHandler_Playeredit(Menu menu, MenuAction action, int client, int item)
{
	switch(action)
	{
		case MenuAction_Select:
		{
			...
			if(StrEqual(cctag[customclient],""))
			{
				char szQuery[512];
				FormatEx(szQuery, sizeof(szQuery), "INSERT INTO `testplayer` (`auth`) VALUES ('%s')",g_szAuth[customclient]);
				g_dDatabase.Query(SQL_CheckForErrors, szQuery);
			}
			PrintToChat(client,"请在聊天栏输入头衔，输入-1取消修改");
			IsEditClient[client]= true;
			EditClient[client]=customclient;
		}
	}
}
...
void SetTag(int client)
{
	...
	{
		char szQuery[512];
		FormatEx(szQuery, sizeof(szQuery), "INSERT INTO `testplayer` (`auth`) VALUES ('%s')",g_szAuth[client]);
		g_dDatabase.Query(SQL_CheckForErrors, szQuery);
	}
	PrintToChat(client,"请在聊天栏输入头衔，输入-1取消修改");
}
...
void UpdateClanTag(int client)
{
	...
	if(gI_RankedPlayers*0.001>=iRank&&iRank>0)
		sCustomTag="[我是传奇]";
	else if(gI_RankedPlayers*0.002>=iRank&&iRank>gI_RankedPlayers*0.001)
		sCustomTag="[神功绝世]";
	else if(gI_RankedPlayers*0.003>=iRank&&iRank>gI_RankedPlayers*0.002)
		sCustomTag="[出神入化]";
	else if(gI_RankedPlayers*0.005>=iRank&&iRank>gI_RankedPlayers*0.003)
		sCustomTag="[登峰造极]";
	else if(gI_RankedPlayers*0.007>=iRank&&iRank>gI_RankedPlayers*0.005)
		sCustomTag="[技冠群雄]";
	else if(gI_RankedPlayers*0.01>=iRank&&iRank>gI_RankedPlayers*0.007)
		sCustomTag="[炉火纯青]";
	else if(gI_RankedPlayers*0.03>=iRank&&iRank>gI_RankedPlayers*0.01)
		sCustomTag="[融会贯通]";
	else if(gI_RankedPlayers*0.05>=iRank&&iRank>gI_RankedPlayers*0.03)
		sCustomTag="[游刃有余]";
	else if(gI_RankedPlayers*0.1>=iRank&&iRank>gI_RankedPlayers*0.05)
		sCustomTag="[运斤成风]";
	else if(gI_RankedPlayers*0.15>=iRank&&iRank>gI_RankedPlayers*0.1)
		sCustomTag="[中流砥柱]";
	else if(gI_RankedPlayers*0.2>=iRank&&iRank>gI_RankedPlayers*0.15)
		sCustomTag="[驾轻就熟]";
	else if(gI_RankedPlayers*0.25>=iRank&&iRank>gI_RankedPlayers*0.2)
		sCustomTag="[渐入佳境]";
	else if(gI_RankedPlayers*0.3>=iRank&&iRank>gI_RankedPlayers*0.25)
		sCustomTag="[以一当十]";
	else if(gI_RankedPlayers*0.4>=iRank&&iRank>gI_RankedPlayers*0.3)
		sCustomTag="[小试牛刀]";
	else if(gI_RankedPlayers*0.6>=iRank&&iRank>gI_RankedPlayers*0.4)
		sCustomTag="[初学乍练]";
	else if(gI_RankedPlayers*0.8>=iRank&&iRank>gI_RankedPlayers*0.6)
		sCustomTag="[新手入门]";
	else if(gI_RankedPlayers>=iRank&&iRank>gI_RankedPlayers*0.8)
		sCustomTag="[凡夫俗子]";
	else if(iRank==0)
		sCustomTag="[萌新]";
	strcopy(RankTag[client],64,sCustomTag);
	
	if(Taguse[client])
	{
		switch(Taguse[client])
		{
			case 0:
			{			
				strcopy(sCustomTag,64,RankTag[client]);						
			}
			case 1:
			{
				strcopy(sCustomTag,64,cctag[client]);						
			}
			case 2:
			{		
				strcopy(sCustomTag,64,"[Admin]");			
			}
		}
	}
...
{
	if (IsEditClient[client])
	{
		char tag[128];
		strcopy(tag, sizeof(tag), sArgs);
		...
		{
			char szQuery[512];
			FormatEx(szQuery, sizeof(szQuery), "UPDATE `testplayer` SET `tag`= '%s' WHERE `auth` = '%s'",tag,g_szAuth[EditClient[client]]);
			g_dDatabase.Query(SQL_CheckForErrors, szQuery);
			PrintToChat(client, "成功将头衔改为为 \x02%s\x01.", tag);
			SQL_FetchPlayer(client);
		}
		else
		{
			PrintToChat(client, "取消了修改头衔");
		}
		IsEditClient[client]=!IsEditClient[client];
